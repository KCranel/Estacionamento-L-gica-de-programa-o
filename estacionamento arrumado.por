programa
{
	...
	inclua biblioteca Texto --> txt
		...
		inteiro horaEstadia[10]
		real caixa = 0.0 
		inteiro carroTot = 0, avulsos = 0, mensalistas = 0, mensalAtivos = 0, hora=7

     funcao inicio()
     {
     	inteiro horaSub=0,contSaida=0,contEntrada=0,horaEnt=0,horaSaida=0
          caracter encerrar
          inteiro opcao
		...
		escreva("\n------------------------")
          ...
          enquanto (encerrar != 'F') {
               escolha (encerrar) {
                    caso 'E':
                         entrada(tipo, placaCarro, col, contVerifica, indis, disp, hora, decisao,horaEnt)
                         pare
                    caso 'S':
                         saida(tipo, col, indis, disp, decisao, horaSaida, horaSub, hora, placaComp)
                    caso 'T':
                         totCarros(decisao)
                    caso 'H':
                    	horas(decisao)
     ...
     funcao entrada(caracter tipo,cadeia placaCarro, inteiro col, inteiro contVerifica, caracter indis, caracter disp, inteiro hora, caracter decisao,inteiro horaEnt)
     {
     ...
     funcao saida(caracter tipo, inteiro col, caracter indis, caracter disp, caracter decisao, inteiro horaSaida, inteiro horaSub, inteiro hora, cadeia placaComp)
     {
     	...
     	inteiro verifPlaca=0
				...
				para(inteiro i=0; i<9;i++){
					...
					se(placaComp==placaEstacionados[0][i]){
						verifPlaca++
					}se(verifPlaca==1){
				...
				leia(col)
					se(carro[1][col]==indis){
						...
						horaSaida = hora
						horaSub = horaSaida-horaEstadia[col]
						
						escreva("\nCarro removido! O valor cobrado sobre o cliente é de ", 15*horaSub, " reais.")
					} senao {
						escreva("\nNão há um carro estacionado nesta vaga.")
						retorne
 ...
 * 
 * @POSICAO-CURSOR = 5028; 
 * @DOBRAMENTO-CODIGO = [89, 78, 220, 225, 241, 252];
 * @PONTOS-DE-PARADA = ;
